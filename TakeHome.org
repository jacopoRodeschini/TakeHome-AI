...
dove $\overline{\theta}$ è il vettore dei parametri ignoti, per cui, la densità di probabilità è proporzionale alla radice quadrata del determinante dell'/informazione di Fisher/. Dalla equazione ref:eqn.bayes se $p(\theta)$ è gaussiana anche $p(\cdot,\theta)$ è gaussiana[fn:3] in quanto risulta essere un prodotto di gaussiane.

Inolte sappiamo che per distribuzioni canoniche esponenziali nella forma con $\theta \in \mathbb{R}$:
...
\[ (\overline{y_{a^*}} + \overline{\epsilon_{a^*}})I/m_{a^*} = (\overline{y_a} + \overline{\epsilon_a})I/m_a) + \xi \tag{23}\]  

possiamo osservare che  $\forall (\xi > 0, \epsilon_{a^*}, y_a_{\in A})$:

\[\forall a \in A : \epsilon_a = [(\overline{y_{a^*}} + \overline{\epsilon_{a^*}})1/ma^* - \overline{y_a}1/ma - \xi]1\tag{24} \]
...
[fn:3] Quando $p(\cdot,theta)$ e $p(\theta)$ hanno la stessa distribuzione si dicono /coniugate/.
